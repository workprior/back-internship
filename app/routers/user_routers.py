...
from app.schemas.user_schemas import User, UserBase, UserCreate, UserSchema, UserUpdateRequest
from app.utils.jwt_utils import encode_jwt

crud_user = APIRouter(prefix="/user")


@crud_user.post("/", response_model=User)
async def create_users(user: UserCreate, session: AsyncSession = Depends(get_session)):
    return await create_user(session, user)


@crud_user.get("/{user_id}/", response_model=User)
async def get_user(user_id: int, session: AsyncSession = Depends(get_session)):
    return await get_user_by_id(session, user_id)


@crud_user.get("s/", response_model=LimitOffsetPage[User])
...
@crud_user.get("/me", response_model=UserBase)
def get_me(current_user: Annotated[UserBase, Depends(get_current_active_user)]):
    return current_user
